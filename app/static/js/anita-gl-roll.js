...
class Anita_GL_Roll {
  constructor(b, c = !1) {
    ...
    (this.loader = new THREE.TextureLoader()),
      ...
      (this.shv = 3.8366),
      (this.spacing =
        this.options.spacing * ((2 * this.shv) / window.innerHeight)),
      (this.position = {
        gallery_t: 0,
        gallery_c: 0,
      ...
      (this.view_pos = { def: 0, zoom: 0, menu: 0 }),
      ...
      jQuery(this.renderer.domElement).addClass("anita-gl-roll-canvas"),
      Anita_isWebGL2Available()
        ? (this.renderTarget = new THREE.WebGLRenderTarget(
            window.innerWidth,
            window.innerHeight,
            { samples: 4 }
          ))
        : (this.this.renderTarget = null),
      ...
      (this.RollShader = {
        uniforms: { tDiffuse: { value: null }, progress: { value: 1 } },
      ...
      (this.rollPass = new THREE.ShaderPass(this.RollShader)),
      this.composer.addPass(this.rollPass);
    let f = 0;
    (this.aGallery = Array()),
      (this.gallery = new THREE.Group()),
      (this.ghosts_lt = new THREE.Group()),
      (this.ghosts_rt = new THREE.Group()),
      this.$el.children("div").each(function (l) {
        let d = jQuery(this),
          c = d.attr("data-size").split("x"),
          m = parseFloat(parseInt(c[0], 10) / parseInt(c[1], 10)).toFixed(3),
          b = a.getImageSize(m),
          e;
        if (d.hasClass("is-video")) {
          let j = jQuery(
            '<video src="' +
              d.attr("data-src") +
              '" webkit-playsinline="true" playsinline="true" muted autoplay loop/>'
          );
        ...
        0 === f
          ? (f += 0.125 * parseFloat(b[1]).toFixed(3))
          : (f -= 0.5 * parseFloat(b[1])),
          (g.position.y = f),
          (h.position.y = f),
          (i.position.y = f),
          (g.position.x = 0),
          (h.position.x =
            0 - parseFloat(b[0]).toFixed(3) - parseFloat(a.spacing)),
          (i.position.x =
            1 * parseFloat(b[0]).toFixed(3) + parseFloat(a.spacing)),
          a.gallery.add(g),
          a.ghosts_lt.add(h),
          a.ghosts_rt.add(i),
          (a.aGallery[l] = f),
          (f -= 0.5 * parseFloat(b[1]) + parseFloat(a.spacing)),
          (g.height = parseFloat(b[1]) + parseFloat(a.spacing));
      }),
      this.scene.add(this.gallery),
      this.scene.add(this.ghosts_lt),
      this.scene.add(this.ghosts_rt),
      (this.active = 0),
      ...
      this.options.nav &&
        ...
        this.$el.after(this.ui.prev).after(this.ui.next),
      ...
      (this.moveEvent = {
        ...
        f_Start: function (b) {
          jQuery(b.target).is("a.anita-gallery-nav")
            ...
            : ((a.moveEvent.active = !0),
              ...
              b.touches
                ? (a.moveEvent.point = b.touches[0].clientY)
                : ((a.moveEvent.point = b.clientY),
        ...
        f_Move: function (c) {
          if ((c.preventDefault(), a.moveEvent.active)) {
            c.touches
              ? (a.moveEvent.path =
                  2 * (a.moveEvent.point - c.touches[0].clientY))
              : (a.moveEvent.path = 2 * (a.moveEvent.point - c.clientY));
            let e = a.moveEvent.path * a.p2p,
              b = -1 * (a.gallery.position.y + e),
              d = 0.5 * a.gallery.children[a.active].height;
            if (
              (a.moveEvent.path > 0 &&
                b <= a.aGallery[a.moveEvent.current + 1] + d &&
                ((a.moveEvent.current += 1), (a.active = a.moveEvent.current)),
              a.moveEvent.path < 0 &&
                b >= a.aGallery[a.moveEvent.current - 1] - d &&
                ((a.moveEvent.current -= 1), (a.active = a.moveEvent.current)),
              (a.position.text_c -= a.moveEvent.path),
              b > a.aGallery[0] || b < a.aGallery[a.max])
            ) {
              if (b > a.aGallery[0] && a.moveEvent.path < 0) {
                let f =
                  (b - a.aGallery[0]) / (0.25 * window.innerHeight * a.p2p);
                (b += a.moveEvent.path * a.p2p * f),
                  (a.position.text_c += a.moveEvent.path);
              }
              if (b < a.aGallery[a.max] && a.moveEvent.path > 0) {
                let g =
                  (a.aGallery[a.max] - b) / (0.25 * window.innerHeight * a.p2p);
                (b += a.moveEvent.path * a.p2p * g),
                  (a.position.text_c += a.moveEvent.path);
              }
            }
            a.$el.css("transform", "translateY(" + a.position.text_c + "px)"),
              (a.ghosts_lt.position.y = -1 * b),
              (a.gallery.position.y = -1 * b),
              (a.ghosts_rt.position.y = -1 * b),
              c.touches
                ? (a.moveEvent.point = c.touches[0].clientY)
                : (a.moveEvent.point = c.clientY);
          }
        },
        f_End: function () {
          a.changeSlide(),
      ...
      jQuery(document)
        .on("mousemove", function (b) {
          if (!a.isTouch && !a.cameraFixed) {
            ...
            (a.position.camera_tM.x = a.options.camera_move[0] * c),
              a.position.camera_tM.x > a.options.camera_move[0] &&
                (a.position.camera_tM.x = a.options.camera_move[0]),
              a.position.camera_tM.x < -1 * a.options.camera_move[0] &&
                (a.position.camera_tM.x = -1 * a.options.camera_move[0]),
              (a.position.camera_tM.y = -1 * (a.options.camera_move[1] * d)),
              a.position.camera_tM.y > a.options.camera_move[1] &&
                (a.position.camera_tM.y = a.options.camera_move[1]),
              a.position.camera_tM.y < -1 * a.options.camera_move[1] &&
                (a.position.camera_tM.y = -1 * a.options.camera_move[1]);
          }
        })
      ...
      jQuery(document).on("keyup", function (b) {
        37 == b.keyCode && a.prevSlide(),
          ...
          38 == b.keyCode && a.prevSlide(),
          40 == b.keyCode && a.nextSlide(),
          32 == b.keyCode &&
            (a.cameraFixed
      ...
      this.$el.parent().hasClass("anita-albums-listing") &&
        ...
        (!0 ==
          ...
          (this.position.gallery_t =
            -1 * this.gallery.children[this.active].position.y),
          (this.gallery.position.y =
            this.ghosts_lt.position.y =
            this.ghosts_rt.position.y =
              this.position.gallery_t),
          (this.position.text_t = this.position.text_c =
            -0.5 * this.active * window.innerHeight),
          this.$el.css(
            "transform",
            "translateY(" + this.position.text_c + "px)"
          )),
  ...
  changeSlide() {
    this.$el.children(".is-active").children("video").length &&
      ...
      (this.position.text_t =
        -1 * this.active * this.$el.children().eq(this.active).height()),
      (this.position.gallery_t =
        -1 * this.gallery.children[this.active].position.y);
  }
  layout() {
  ...
  animate() {
    ...
    (this.prevTime = c),
      b.moveEvent.active ||
        ((this.gallery.position.y +=
          (this.position.gallery_t - this.gallery.position.y) * 2 * a),
        (this.ghosts_lt.position.y +=
          (this.position.gallery_t - this.ghosts_lt.position.y) * 2 * a),
        (this.ghosts_rt.position.y +=
          (this.position.gallery_t - this.ghosts_rt.position.y) * 2 * a)),
      this.cameraFixed
        ? ((this.camera.rotation.x += (0.07 - this.camera.rotation.x) * 2 * a),
          (this.camera.rotation.z += (0 - this.camera.rotation.z) * 2 * a))
        : ((this.camera.rotation.x += (0 - this.camera.rotation.x) * a),
          (this.camera.rotation.z +=
            (this.position.camera_tR.z - this.camera.rotation.z) * a),
          this.camera.rotation.x > 0.07
            ? (this.camera.rotation.x = 0.07)
            : this.camera.rotation.x < -0.3 && (this.camera.rotation.x = -0.3),
          this.camera.rotation.z > 0.05
            ? (this.camera.rotation.z = 0.05)
            : this.camera.rotation.z < -0.05 &&
              (this.camera.rotation.z = -0.05)),
      this.cameraFixed
        ? ((this.camera.position.x += (0 - this.camera.position.x) * 2 * a),
          (this.camera.position.y += (-0.3 - this.camera.position.y) * 2 * a),
          (this.camera.position.z +=
            (this.view_pos.zoom - this.camera.position.z) * 2 * a))
        : jQuery("body").hasClass("anita-show-menu")
        ? ((this.camera.position.x +=
            (this.position.camera_tM.x - this.camera.position.x) * a),
          (this.camera.position.y +=
            (this.position.camera_tM.y - this.camera.position.y) * a),
          (this.camera.position.z +=
            (this.view_pos.menu - this.camera.position.z) * a * 2))
        : ((this.camera.position.x +=
            (this.position.camera_tM.x - this.camera.position.x) * a),
          (this.camera.position.y +=
            (this.position.camera_tM.y - this.camera.position.y) * a),
      ...
      this.camera.position.x > this.options.camera_move[0]
        ? (this.camera.position.x = this.options.camera_move[0])
        : this.camera.position.x < -1 * this.options.camera_move[0] &&
          (this.camera.position.x = -1 * this.options.camera_move[0]),
      this.camera.position.y > this.options.camera_move[1]
        ? (this.camera.position.y = this.options.camera_move[1])
        : this.camera.position.y < -1 * this.options.camera_move[1] &&
          (this.camera.position.y = -1 * this.options.camera_move[1]),
      (this.rollPass.uniforms.progress.value +=
        (this.position.curve_t - this.rollPass.uniforms.progress.value) *
        2 *
        a),
      this.rollPass.uniforms.progress.value > 1
        ? (this.rollPass.uniforms.progress.value = 1)
        : this.rollPass.uniforms.progress.value < 0 &&
          (this.rollPass.uniforms.progress.value = 0);
    let d = 0;
    jQuery(this.gallery.children).each(function () {
      ...
      this.gallery.children[this.active].material.uniforms.fadeLevel.value <
        ...
        (this.gallery.children[this.active].material.uniforms.fadeLevel.value +=
          ...
          0.05),
      b.moveEvent.active ||
        ((this.position.text_c +=
          (this.position.text_t - this.position.text_c) * 2 * a),
        this.$el.css(
          "transform",
          "translateY(" + this.position.text_c + "px)"
        )),
