    ...
    document.addEventListener(
      ...
      function (e) {
        if (!this_class.ax) {
          ...
          if (this_class.xs) {
            this_class.xd =
              this_class.xd + Math.abs(this_class.xs - e.touches[0].clientX);
            this_class.xs = e.touches[0].clientX;
          }
          if (this_class.ys) {
            this_class.yd =
              this_class.yd + Math.abs(this_class.ys - e.touches[0].clientY);
            this_class.ys = e.touches[0].clientY;
          }
...
class Anita_Before_After {
  constructor($obj) {
    if ($obj instanceof jQuery) {
      ...
      this.$el = {
        $wrap: $obj,
        $before: jQuery(
          '<div class="anita-before-after-img anita-before-img"/>'
        ).appendTo($obj),
        $after: jQuery(
          '<div class="anita-before-after-img anita-after-img-wrap"/>'
        ).appendTo($obj),
        $divider: jQuery(
          '<div class="anita-before-after-divider">\
  ...
  layout() {
    ...
    this.$el.$after
      .children(".anita-after-img")
      .width(this_class.$el.$wrap.width())
      .height(this_class.$el.$wrap.height());
  }
  reset() {
    this.current = 50;
    this.target = 50;
...
class Anita {
  constructor(cfg = false) {
    ...
    this.options = {
      ...
      int_cursor: true,
      cursorHover:
        'a, button, input[type="submit"], .anita-before-after, .anita-toggles-item--title, .anita-hover',
      cursorscrollEW: ".anita-scrollEW, .pswp__scroll-wrap, .owl-stage-outer",
      cursorscrollNS: ".anita-scrollNS",
      cursorFollow:
        ".anita-menu-toggler, .anita-back2top, .anita-socials-list a, .owl-dot, .anita-gallery-nav, .pswp__button",
    ...
    if (_self.options.back2top) {
      _self.$el.b2t = jQuery(
        '<a href="javascript:void(0)" class="anita-back2top"></a>'
      )
  ...
  contentOrigin() {
    if (this.$el.main.children(".anita-container").length) {
      this.$el.main
        .children(".anita-container")
        .css(
          "transform-origin",
          "50% " + (window.scrollY + 0.5 * window.innerHeight) + "px"
        );
    }
  }
  generateID() {
  ...
  init() {
    ...
    this.maxScroll =
      this.$el.body.height() - this.$el.footer.height() - this.$el.win.height();
    if (this.$el.b2t) {
    ...
    if (_self.options.disable_right_click) {
      ...
      _self.$el.body.append(
        '\
          ...
          '</p>\
                    <a href="#" class="anita-rcpm-close anita-button">' +
      ...
      if (jQuery(".anita-rcp-message").length) {
        jQuery(".anita-rcp-message").on(
          "click",
          ".anita-rcpm-overlay, .anita-rcpm-box",
          function (e) {
            e.preventDefault();
            _self.$el.body.removeClass("anita-rcp-message-show");
          }
    ...
    let anita_fullscreen_menu = function () {
      ...
      let count = 1;
      _self.$el.nav
        .children("ul.main-menu")
        .children("li")
        .each(function () {
          jQuery(this)
            .addClass("is-hidden")
            .children("a")
            .prepend("<sup>" + (count < 10 ? "0" + count : count) + ".</sup>");
          count++;
        });
      ...
      _self.$el.nav.show_menu = function () {
        ...
        let items_length =
          _self.$el.nav.children("ul.main-menu").children("li").length - 1;
    ...
    let anita_mobile_menu = function () {
      // Create Mobile Menu
      _self.$el.mobileMenu = jQuery(
        '<div class="anita-mobile-menu-wrap anita-fullscreen-menu-wrap"/>'
      ).appendTo(_self.$el.body);
      ...
      // Mobile Menu Button
      _self.$el.header
        .find(".anita-menu-wrapper")
        .append(
          '<a href="#" class="anita-menu-toggler anita-mobile-menu-toggler"><i class="anita-menu-toggler-icon"></i></a>'
        );
    ...
    let anita_js_menu = function () {
      if (_self.options.main_menu !== null) {
        ...
        let get_submenu = function (label, obj, $parent) {
          let $li = jQuery('<li class="menu-item-has-children"/>').appendTo(
            $parent
          );
    ...
    if (jQuery(".anita-js-socials").length && _self.options.socials) {
      jQuery(".anita-js-socials").each(function () {
        let $socials = jQuery('<ul class="anita-socials-list"/>').appendTo(
          this
        );
        for (const [classname, item] of Object.entries(_self.options.socials)) {
          $socials.append(
            '<li class="anita-socials--' +
              classname +
              '"><a href="' +
              item.url +
              '">' +
    ...
    if (jQuery(".anita-lightbox-link").length) {
      ...
      jQuery(document).on("click", ".anita-lightbox-link", function (e) {
        ...
        if (typeof PhotoSwipe == "function") {
          ...
          _self.pswp.lightbox = new PhotoSwipe(
            _self.$el.body.find(".pswp")[0],
          ...
          if ($this.attr("data-type") !== "image") {
            let $this_video = jQuery(_self.pswp.lightbox.container).find(
              '[data-src="' + $this.attr("href") + '"]'
            );
            $this_video
              .addClass("is-inview")
              .width(_self.pswp.resizeVideo().w)
              .height(_self.pswp.resizeVideo().h);
          ...
          _self.pswp.lightbox.listen("resize", function () {
            if (
              jQuery(_self.pswp.lightbox.container).find(
                ".anita-pswp-media--video"
              ).length
            ) {
              jQuery(_self.pswp.lightbox.container)
                .find(".anita-pswp-media--video")
                .width(_self.pswp.resizeVideo().w)
                .height(_self.pswp.resizeVideo().h);
            }
            if (
      ...
      this.cursor = {
        ...
        init: function () {
          ...
          if (window.localStorage.getItem("anita_prev_cursor") !== null) {
            let prevCursor = JSON.parse(
              window.localStorage.getItem("anita_prev_cursor")
            );
        ...
        render: function () {
          if (this.isActive) {
            ...
            this.position.lerpX +=
              (this.position.x - this.position.lerpX) * 10 * deltaTime;
            this.position.lerpY +=
              (this.position.y - this.position.lerpY) * 10 * deltaTime;
            this.$el.css(
              "transform",
              "translate(" +
                this.position.lerpX +
      ...
      if (this_href.indexOf("javascript") === 0) {
        ...
        if (
          ...
          if (_self.$el.main.find($el[0].hash).length) {
            ...
            jQuery("html, body")
              .stop()
              .animate({ scrollTop: scrollPos }, 300, function () {
                _self.$el.body.removeClass("is-locked");
                if (_self.cursor) {
                  _self.cursor.unsetState("is-busy");
                }
  ...
  init_page() {
      ...
      _self.$el.main.find(".anita-albums-back").each(function () {
        ...
        if ($this.attr("href") == "#") {
          if (window.localStorage.getItem("anita_listing_url") !== null) {
            jQuery(".anita-albums-back").attr(
              "href",
              window.localStorage.getItem("anita_listing_url")
            );
          } else {
    ...
    if (jQuery(".anita-data-background[data-src]:not(.is-loaded)").length) {
      jQuery(".anita-data-background[data-src]:not(.is-loaded)").each(
        function () {
          let $this = jQuery(this);
          $this
    ...
    if (jQuery(".anita-page-background[data-src]:not(.is-loaded)").length) {
      jQuery(".anita-page-background[data-src]:not(.is-loaded)").each(
        function () {
          let $this = jQuery(this);
    ...
    if (jQuery("[data-opacity]:not(.is-loaded)").length) {
      jQuery("[data-opacity]").each(function () {
        jQuery(this)
          .css("opacity", jQuery(this).attr("data-opacity"))
          .addClass("is-loaded");
      });
    }
    ...
    // Form Fields
    jQuery(
      'input:not(.is-init):not([type="submit"]):not([type="reset"]):not([type="button"]), textarea:not(.is-init)'
    ).each(function () {
      let $this = jQuery(this);
    ...
    // Testimonials
    if (
      jQuery(".anita-testimonials-grid:not(.is-init):not(.disable-hovers)")
        .length
    ) {
      jQuery(".anita-testimonials-grid:not(.is-init)").each(function () {
        ...
        $this.addClass("is-init");
        $this
          .on("mouseenter", ".anita-testimonials-item", function () {
            if (!_self.isTouchDevice) {
      ...
      jQuery(".anita-lightbox-link").each(function () {
        ...
        if ($this.attr("data-type") && $this.attr("data-type") !== "image") {
          ...
          if ("video" == $this.attr("data-type")) {
            this_item.html =
              '\
        				<div class="anita-pswp-media--video" data-src="' +
              $this.attr("href") +
              '">\
        					<video src="' +
              $this.attr("href") +
            ...
            this_item.html =
              '\
        				<div class="anita-pswp-media--iframe" data-player-src="' +
              this_link +
              '" data-src="' +
              $this.attr("href") +
              '" data-type="' +
    ...
    if (jQuery(".anita-flat-carousel:not(.is-init)").length) {
      let anita_flat_carousel_init = function () {
        jQuery(".anita-flat-carousel:not(.is-init)").each(function () {
          ...
          let options = {
            size: parseFloat(
              $this.attr("data-size") ? $this.attr("data-size") : 0.5,
              10
            ),
            spacing: $this.attr("data-spacing")
              ? $this.attr("data-spacing")
              : 40,
          };
          $this.addClass("is-init");
